...
from .ops import duplicate, remove, reset_opa, split, add_from_completion
...
class DefaultStrategy(Strategy):
    ...
    def add_gs_from_completion(
        self,
        params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
        optimizers: Dict[str, torch.optim.Optimizer],
        compl_gs_xyz,        # [M,3]
        compl_gs_scales,     # [M,3] (log-scale)
        compl_gs_opacities,  # [M,1] (logit alpha)
        compl_gs_quats,      # [M,4] (w,x,y,z)
        compl_gs_f_dc,       # [M,1,3]
        compl_gs_f_rest,     # [M,C_sh,3]
        step: int
        ):
        add_from_completion(    
            params,
        ...
        # print(f"step {step} add {compl_gs_xyz.shape[0]} gs ball!")
    ...
    def step_post_backward(
        ...
        Args:
            mask (torch.Tensor, optional): A boolean mask of shape (N,) indicating 
                which Gaussians were used in the current step. This mask will be 
                accumulated until the next refinement step.
        """
        if step >= self.refine_stop_iter:
            return

        self._update_state(params, state, info, packed=packed, mask=mask)
        ...
        if (
            ...
            # reset running stats and accumulated mask
            state["grad2d"].zero_()
            state["count"].zero_()
            if self.refine_scale2d_stop_iter > 0:
                state["radii"].zero_()
            if "mask_accum" in state:
                state["mask_accum"] = None
    ...
    def _update_state(
        ...
        # Initialize accumulated mask if not exists
        if "mask_accum" not in state:
            state["mask_accum"] = None
        if state["mask_accum"] is None:
            state["mask_accum"] = torch.zeros(n_gaussian, dtype=torch.bool, device=grads.device)
        ...
        # Update accumulated mask if provided
        if mask is not None:
            if mask.dtype != torch.bool:
                mask = mask.bool()
            # Ensure mask has the right shape
            if len(mask) != n_gaussian:
                raise ValueError(f"mask length {len(mask)} doesn't match number of Gaussians {n_gaussian}")
            # Accumulate: mark any Gaussian used in any step
            state["mask_accum"] = torch.logical_or(state["mask_accum"], mask)
    ...
    def _grow_gs(
        ...
        # Optional: only grow Gaussians that were actually used (according to accumulated mask)
        if "mask_accum" in state and state["mask_accum"] is not None:
            is_dupli = is_dupli & state["mask_accum"]
            is_split = is_split & state["mask_accum"]
            n_dupli = is_dupli.sum().item()
            n_split = is_split.sum().item()
    ...
    def _prune_gs(
        ...
        # Optional: only prune Gaussians that were actually used (according to accumulated mask)
        if "mask_accum" in state and state["mask_accum"] is not None:
            # We might want to keep unused Gaussians or prune them differently
            # Here we keep unused Gaussians (don't prune them based on opacity/size)
            is_prune = is_prune & state["mask_accum"]
