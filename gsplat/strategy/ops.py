...
def add_from_completion(
    ...
    M      = compl_gs_xyz.shape[0]

    xyz_new    = compl_gs_xyz.to(device=device, dtype=dtype)
    scales_new = compl_gs_scales.to(device=device, dtype=dtype)
    opac_new   = compl_gs_opacities.to(device=device, dtype=dtype).squeeze(-1)
    quats_new  = compl_gs_quats.to(device=device, dtype=dtype)
    quats_new  = quats_new / (quats_new.norm(dim=-1, keepdim=True) + 1e-8)

    sh0_new = compl_gs_f_dc.to(device=device, dtype=dtype)         # [M,1,3]
    shN_new = compl_gs_f_rest.to(device=device, dtype=dtype)       # [M,C,3] 或 [M,0,3]
    ...
    def param_fn(name, old_param):
        if name in key_map and key_map[name] is not None:
            addon = key_map[name]
            if old_param.dim() == addon.dim() and list(old_param.shape[1:]) == list(addon.shape[1:]):
                p_new = torch.cat([old_param, addon], dim=0)
                return torch.nn.Parameter(p_new, requires_grad=old_param.requires_grad)
        return old_param

    def optimizer_fn(key, v):
        zeros = torch.zeros((M, *v.shape[1:]), device=v.device, dtype=v.dtype)
